...
| Step 4: Voice Mode (Realtime API) | in-progress | M1 complete (WebRTC connection, voice conversation, auto-greeting). M2 + M3 pending. |
...
1. Step 4 M2: Add voice tool-call handling — cart updates when AI processes orders via voice
2. Step 4 M3: Order finalization via voice, error handling, mic denial UX, disconnect cleanup
3. Step 5: Rebrand UI from Delo to NYC cafe visual system
4. Step 7: Upgrade owner dashboard visualizations and metrics depth
...
- **Voice mode**: Uses inline `VOICE_INSTRUCTIONS` constant via Realtime API — `gpt-4o-mini-realtime-preview`
